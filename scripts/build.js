...
const REPOSITORY_NAME = 'cloneHouse';
...
function copyFavicons() {
  faviconFileList.forEach((filename) => {
    copySync(path.resolve(__dirname, `../${filename}`), path.join(buildDir, filename));
  });
}
...
async function build() {
  ...
  copySync(path.resolve(__dirname, '../index.html'), path.join(buildDir, 'index.html'));
  copySync(path.resolve(__dirname, '../js'), path.join(buildDir, 'js'));
  copySync(path.resolve(__dirname, '../assets/fonts'), path.join(buildDir, 'assets/fonts'));
  copyFavicons();
