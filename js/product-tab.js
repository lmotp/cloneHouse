...
let currentActvieTab = productTab.querySelector('.is-active');

const TOP_HEADER_DESKTOP = 80 + 50 + 54;
const TOP_HEADER_MOBILE = 50 + 40 + 40;

let disalbedUpdating = false;

function toggleActiveTab() {
  const tabItem = this.parentNode;

  if (currentActvieTab !== tabItem) {
    disalbedUpdating = true;
    tabItem.classList.add('is-active');
    currentActvieTab.classList.remove('is-active');
    currentActvieTab = tabItem;

    setTimeout(() => {
      disalbedUpdating = false;
    }, 1000);
  }
}
...
function updateActiveTabOnScroll() {
  ...
  if (newActiveTab) {
    ...
    if (newActiveTab !== currentActvieTab) {
      newActiveTab.classList.add('is-active');
      if (currentActvieTab !== null) {
        currentActvieTab.classList.remove('is-active');
      }
      currentActvieTab = newActiveTab;
    }
  }
}
